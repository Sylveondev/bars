...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('dehoist')
		.setDescription('Dehoist a user\'')
		.addUserOption(option=>option.setName("user").setDescription("The user to dehoist").setRequired(true)),
	async execute(interaction) {
		...
		var n = validuser.nickname||user.username
		validuser.setNickname("឵"+n)
		.then(()=>{interaction.followUp(`:ok_hand: | Successfully dehoist **${user.tag}**'.`)})
		.catch(e=>{interaction.followUp(":x: | Unable to dehoist user.")})
	}
};